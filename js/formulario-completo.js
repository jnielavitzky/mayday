...
$(document).ready(function() {
    ...
    phones_max = 1; //initial text box count
    $(add_button).click(function(e) { //on add input button click
        e.preventDefault();
        if (phones_max < max_fields) { //max input box allowed
            phones_max++; //text box increment
            $(wrapper).append('<div><input type="text" name="mytext[]" id="phone' + phones_max + '"  class="fill-in phone"/><a href="#" class="remove-field">&nbsp;Quitar telefono</a></div>'); //add input box
        }
    });
...
$(document).ready(function() {
    ...
    for (i = 0; i < pasajeros; i++) {
        var appendable = '<div class="row">' +
            ...
            '<br>' +
            '<select class="seleccion" name="seleccion_doc' + i + '" id="seleccion_doc' + i + '">' +
...
function getData() {
    var data = {};
    data["flight_id"] = 93480;

    var passengers = [];
    for (var i = 0; i < pasajeros; i++) {
        var passanger1 = {};
        passanger1["first_name"] = $("#nombre" + i).val();
        passanger1["last_name"] = $("#apellido" + i).val();
        passanger1["birthdate"] = $("#año" + i).val() + "-" + $("#mes" + i).val() + "-" + $("#dia" + i).val() //"1969-06-02";
        passanger1["id_type"] = ($("#seleccion_doc" + i).val() == "DNI") ? 1 : 2;
        passanger1["id_number"] = $("#nro-documento-pasajero" + i).val();
        passengers.push(passanger1);
    }
    data["passengers"] = passengers;

    var payment = {};
    payment["installments"] = parseInt($("#cuotas").val(), 10);

    var credit_card = {};
    credit_card["number"] = $("#fill-in-tarjeta").val();
    credit_card["expiration"] = $("#vencimiento-mes").val() + "" + ($("#vencimiento-año").val() - 2000);
    credit_card["security_code"] = $("#codigo-de-seguridad").val();
    credit_card["first_name"] = $("#nombre-p").val();
    credit_card["last_name"] = $("#apellido-p").val();
    ...
    billing_address["zip_code"] = parseInt($("#cp").val(), 10);
    billing_address["street"] = $("#calle").val() + "" + $("#numero-casa").val();
    billing_address["floor"] = $("#piso-casa").val();
    billing_address["apartment"] = $("#dpto-casa").val();
    ...
    var contact = {};
    contact["email"] = $("#correo_electronico").val();
    var phones = [];

    for (var i = 0; i < phones_max; i++) {
        var phone_id = "#phone" + ((i != 0) ? (i+1) : "");
        phones.push($(phone_id).val());
    }
